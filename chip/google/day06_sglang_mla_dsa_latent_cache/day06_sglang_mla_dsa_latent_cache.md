...
此前接触过的 vLLM Full Attention page/block/KV 生命周期只作为概念背景（本目录的 Day05 实际聚焦 Mosaic 编译链），今天不再重复原理。前 30 分钟只完成正式 SGLang 的差异闭环；随后把主角切到 MLA latent cache，并把 DSA 看成“MLA 主状态 + 与它共址、同寿命的 indexer K sidecar”。
...
先看 DeepSeek 官方论文里的整体架构图：绿色路径是 lightning indexer → top-k selector → MLA core attention；它解释“为什么需要 indexer sidecar”，但没有画 SGLang 的 allocator、Radix ownership 或 page 生命周期。下面的自绘图才把这些运行时状态补齐。

![DeepSeek-V3.2-Exp 官方 DSA under MLA 架构图](assets/01_deepseek_dsa_mla_official.png)

来源：[DeepSeek-V3.2-Exp 官方仓库与论文](https://github.com/deepseek-ai/DeepSeek-V3.2-Exp)，[固定 commit `87e509a`](https://github.com/deepseek-ai/DeepSeek-V3.2-Exp/tree/87e509a2e5a100d221c97df52c6e8be7835f0057)。这是论文第 2 页 Figure 1 的本地裁剪副本；原 PDF、裁剪坐标与许可记录见 `assets/REMOTE_IMAGES.md`。

![MLA / DSA 完整状态总线（总结图/自绘）](assets/mla_dsa_lifecycle.svg)

读图先从上到下，而不是从左到右追某个 Python 函数。第一层是启动期物理内存：容量一旦确定，主 latent tensors 与 DSA sidecar tensors 都一次性创建；第二层是请求期地址关系：prefix hit 与新分配位置拼成 `ReqToTokenPool` 的一行；第三层是每轮 forward：当前 token 的投影结果沿红色实线写入同一 `out_cache_loc`，backend 再沿蓝色虚线把 request row 翻译为 kernel view；最下层是生命周期：finish 按 `is_insert` 决定是否插入缓存，释放重复/尾部位置并转移（或释放）ownership，evict 才把仍在 Radix 中的 page ID 交回 allocator；这两种操作都不会销毁那块 CUDA tensor。
...
## 二、30 分钟闭环：从概念背景到正式 SGLang，究竟差在哪

### 2.1 最大差异：核心表不是 logical-block table，而是逐 token flat-location table

vLLM 的典型直觉是 `request logical block -> physical block id`，token slot 通常还要由 `block_id * block_size + offset` 展开。正式 SGLang 的 `ReqToTokenPool` 更直接：GPU tensor 的每一行对应一个 request slot，每一列对应该 request 的 token position，每个值已经是 physical KV pool 的 flat slot。
...
![SGLang 与 vLLM 的映射差异（总结图/自绘）](assets/sglang_full_attention_delta.svg)
...
以下段落限定在此快照的 non-Eagle、token-key Radix 路径。Eagle 模式会把 `RadixKey` 转成 bigram 逻辑单元：原始 token 数为 `N+1` 时，缓存 value 只有 `N` 个 logical units；因此不能把“key/value 同长度”这句话直接套到 Eagle。

在 non-Eagle 路径中，Radix node 的 key 是 token-id segment 与 namespace 信息，value 是同长度、page-aligned 的 physical KV indices。`match_prefix` 返回的是可以直接放入 request row 的 loc tensor，不是“命中了一个逻辑块，稍后再查物理块”；匹配结果交给上层调度后，调用方才会对终止 Radix 路径加锁。如果命中 64 个 token，最重要的副作用不是复制 64 行 KV，而是把原有 64 个 loc 复用到新 request 的 mapping。
...
如果此前形成了“free queue 里的 block 仍可保留 cache identity”的 vLLM 直觉，今天必须清掉。此 SGLang 路径里，一个 cached Radix node 即使 `lock_ref==0`、允许被驱逐，它的 pages 仍没有进入 allocator 的 `free_pages`；只有 `RadixCache.evict()` 删除叶节点并调用 allocator free 后，page ID 才重新可分配。
...
|---|---|---|---|---|
| match 后（调用方加锁） | `prefix_indices=704..767`，`last_node` 被锁 | 尚未绑定或只待写 | page 11 cached-protected | page 11 K/V 已存在 |
...
| finish | request 即将释放 row 23 | row 内容可变成 stale metadata | `is_insert` 时才把对齐部分插入/复用 Radix；重复副本与未对齐尾按规则 free，活动锁释放 | cached pages 继续常驻；free pages 等待覆盖 |
...
为避免把量化变体混进主线，下面的 shape/字节示例采用 **unpacked、非量化 BF16 MLA teaching path**。真实运行时的 KV dtype/layout 由配置和 backend 决定；FP8/FP4 等 packed/quantized cache 可能带额外 scale、packing 或不同 row shape，必须按对应 contract 解读。
...
第一式把 K 的 up-projection 吸收到 query 一侧，第二式把 V 的 up-projection 推迟到 attention reduce 之后。因此 persistent storage 不必保存每个 head 展开的 K/V。RoPE 分量仍随 token/position 变化并参与 score，所以 `k_rope` 必须跟 latent 一起缓存。

### 3.2 SGLang 的真实 physical layout

默认 MLA pool 为每个本地 attention layer 创建一个 tensor（以下示例沿用上面的 unpacked BF16 teaching path）：
...
| prefix match + 上层加锁 | `RadixCache` + scheduler | 返回 prefix locs，并锁定终止路径 | 无 latent copy | 命中 loc 在 allocator 看来绝不能 free |
...
| finish | release + `RadixCache` | 按 `is_insert` 可选转交对齐 prefix；释放 row/lock/未缓存部分 | 通常无 latent copy | 已缓存 loc 不进 `free_pages`；未插入部分按规则回收 |
...
physical shape = [num_pages, page_size * 128 + page_size * 4] uint8
               = [num_pages, 64 * 128 + 64 * 4] uint8
               = [num_pages, 64 * 132] uint8  # capacity accounting

physical page layout (SoA, not an interleaved struct):
  K region:     [64, 128] uint8  at byte offset 0
  scale region: [64, 4]   uint8  (FP32 scale bytes) at byte offset 64*128
logical token i pairs K[i, :] with scale[i, :]
```

这里的 `132 B/token` 只表示同一 token 的 K/scale 配对在容量计算上的逻辑成本；物理 page 先连续存完整 K 区，再连续存完整 scale 区。同一 token 通过相同的 page offset 配对两区，内存中不是 `[128-byte key | 4-byte scale]` 逐 token 交错排列。
...
| 项目 | 计算 | 结果 |
|---|---:|---:|
| indexer 一 token / layer（逻辑配对） | `128 B + 4 B` | 132 B |
| indexer 一 page / layer（物理 K 区 + scale 区） | `64×128 B + 64×4 B` | 8,448 B = 8.25 KiB |
...
SGLang 的做法是让 `DSATokenToKVPool` 同时拥有 main `kv_buffer` 与 `IndexKeyCache`，二者接受同一 `forward_batch.out_cache_loc`。allocator 只管理一套 page IDs，Radix value 只保存一套 locs，pool 的 move 路径也要求 latent 与 indexer 一起移动。于是“某个 loc 当前属于哪个 token-prefix 世代”成为单一事实来源。这里的“世代”是正确性约束/测试概念；源码显式的 `req_generation` 只记录 request row 的复用代数，并不是另设一份 KV-page epoch。
...
对每个 query token，indexer 从历史 sidecar 读 key rows，计算相关性并取 top-k。随后 `topk_transform` 把“score 矩阵的列号”翻成 downstream sparse attention 认可的地址：fused legacy 的 PAGED 路径通过 size-1 page table 映射到 physical locations，RAGGED 路径加上 flattened KV offsets；真正的 unfused 分支返回当前有效窗口内的 local indices，也就是 selected score columns 减去 `row_starts`（`row_starts=None` 时才等于原始列号），由后续 consumer 再按 contract 解释；CUDA 上满足 top-k v2 条件的 fused PAGED 路径则直接消费 `real_page_table`，不物化 size-1 table。源码接口明确警告调用者不要假设返回值永远是原始 logits 的列索引；正确抽象是“backend-specific, kernel-consumable candidate locations”。
...
decode batch 每个 request 通常新增一个 token location。allocator 若当前 page 尚有空位就沿用该 page 的下一个 offset，否则拿一个新 page；`ReqToTokenPool[R, old_seq_len]` 写入新 loc。所有 DSA layers 对该 loc 写本层 latent row；只有拥有 indexer、会自行计算 top-k 的 producer layer 才写本层 indexer row，`skip_topk` layer 复用前层结果并保留 0-row sidecar。随后 backend 结合 request rows 和有效长度构造两种 table：在 eager/legacy 路径中，`page_table_1` 保留逐 token flat locations，`real_page_table` 每 64 token 取 page ID，供 page-oriented indexer/attention path 使用；满足 fused top-k v2 条件的 CUDA graph decode 可以省掉 `page_table_1`，直接从 `req_to_token` 填充 compact `real_page_table`。

kernel-visible DSA interface 可以分两段记。indexer 得到当前 query/indexer weights、indexer buffer pointer、真实或 size-1 page table、sequence lengths/valid ranges，输出 backend-specific candidate representation（可能是 transformed top-k，也可能是 unfused window-local indices）；attention 得到 query latent/RoPE、main latent buffer pointer、top-k locations、稀疏有效长度与必要 scheduler metadata，读取被选 rows 并完成 attention。`out_cache_loc` 是本轮两份 persistent writes 的地址，`topk` 是本轮 reads 的地址，两者不要混淆。

### 4.6 prefix reuse 在 DSA 中成立的条件

DSA prefix cache 可以成立，因为对固定模型权重、RoPE 配置、KV dtype/layout、adapter/LoRA 和 cache namespace，历史 token 的 latent row 与 indexer key row 都是 token prefix 的确定性函数。Radix hit 返回旧 locs 后，新 request 直接复用两块 storage 对应的同一坐标，不做数据复制。
...
`page_table_1` 是按 batch row gather 出来的逐 token flat loc table，形状可看作 `[2,max_k_len]`，尾部无效内容由 lengths 屏蔽；`real_page_table` 则是 page-oriented view，A 的有效 pages 是 `[11,41]`，B 是 `[52]`。在本小节假设四个 layer 都是 indexer producer 的教学配置下，主池**每层**写 16 条 `[1,576]` BF16 rows、四层合计 64 条；sidecar 同样每层写 16 条逻辑 132-byte 配对（物理上是连续 K 区与 scale 区）、四层合计 64 条。若某层是 `skip_topk`，该层只有主池写入，sidecar 为 0-row placeholder。由于 allocator 粒度是 page，本轮虽然只写 page 41 的 6 行、page 52 的 10 行，但两个 page 的 ownership 已完整交给对应 request；这不发生新的 CUDA allocation，只是占用预分配 pool 的两个 page IDs。

在教学 `topk=4` 且 `row_starts=0` 的例子中，A 某个 query 的 window-local top-k 可以是 score columns `[0,25,51,68]`；在 legacy fused PAGED transform 中，需要再通过 A 的 size-1 table 得到对应 physical locs `[704,729,755,2628]`（fused top-k v2 则由 compact real-page table 完成同一映射；真正 unfused 分支返回 window-relative local indices，只有本例因起点为 0 才与原始列号相同）。B 的同名 token 或下一步 query 会产生不同 top-k。这个例子只演示地址转换，不声称真实模型的 top-k 配置为 4。
...
| request finish | row 23/31 可释放 | 对齐 prefix 可按 `is_insert` 转给 Radix，否则释放 | sidecar 与同 loc 主状态一起转交或失效 | 不缓存 |
...
看完必须知道：match 先对 key 做 page alignment；finish 按 `is_insert` 选择是否插入对齐部分，再释放重复/尾部并减锁；evict 只选可驱逐叶并把它们的 locs 交回 allocator。
...
调用与状态解释：请求 prefix lookup 调 `match_prefix`，上层随后锁定匹配路径；finish/release 调 `cache_finished_req`，内存紧张时 allocation path 触发 `evict`；输入分别是 token namespace、request row 中的 locs 和所需 token 数；它们改变 allocator/cache metadata 与 request lock/row ownership，通常不搬动 physical bytes；输出命中 locs 或已驱逐 token 数；下一站继续 allocation/reuse。关键 invariant 是一个 cached loc 要么由 protected/evictable Radix node 持有，要么已从树删除后才可 allocator-free，绝不可两边同时拥有。

读后自检：finish 后 row 被 free，为何 node value 还能安全读？因为 node 已复制 loc metadata 并接管对齐 pages 的 cache ownership；复制的是 indices，不是 latent bytes。
...
看完必须知道：模型投影输出按 `q_lora_rank` 与 `kv_lora_rank+rope_dim` 切分；DSA layer 可调用 indexer；attention 以 `k_nope` 同时充当 latent K/V 接口；FlashMLA backend 从 request rows 构造 block table，并传入 buffer view、lengths 和 scheduler metadata。
...
调用与状态解释：decoder layer 调模型 attention，后者调 `RadixAttention` 再分派 backend；输入是 hidden/query projections 与 `ForwardBatch`；模型阶段形成本轮 payload，backend setter 写 physical rows，metadata builder 只创建 per-forward views，attention 调用返回当前 token outputs；下一站是后续 projection/layer。关键 invariant 是 storage dims、query absorbed dims、backend `head_dim_v` 和 buffer view 对同一 latent layout 达成一致，且 kernel 的 table/lengths 只暴露当前 request 的有效 rows。
...
看完必须知道：`DSATokenToKVPool` 把 sidecar 包在同一 pool owner 中；`IndexKeyCache` 的真实 shape 是 page-major bytes；在 eager/legacy metadata 中，`DSAMetadata` 同时保留 size-1 和真实 page table（满足 fused top-k v2 的 CUDA graph decode 可省略前者）；indexer 用 `out_cache_loc` 存 K+scale，并把 top-k 变为 downstream 可消费的 candidate representation。
...
调用与状态解释：模型 attention 调 Indexer，Indexer 通过全局/current pool facade 取得 sidecar；输入是当前 hidden/q-lora/positions、layer id 与 `ForwardBatch`；store 改 physical indexer state，DSA backend init 改 per-forward metadata，top-k transform 只产生临时 kernel-visible locations；输出是当前 query 的 candidate representation，下一站由 sparse attention 读 main latent rows。关键 invariant 是 main latent 与 indexer sidecar 对每个可见 loc 属于同一 generation，而 top-k 返回值必须按 backend contract 解释，不能被长期缓存或一概当作 raw score index。
...
- 跳过 speculative decoding、CUDA graph replay、DCP/CP、layer-split、HIP/NPU 与 unified-memory 分支，因为它们会扩展 loc space、静态 metadata buffer 或 layer ownership，却不会改变“先解析 loc，再按 pool layout 读写”的核心 boundary；源码范围中看到这些分支时只确认它们没有绕开主 invariant。
...
- 跳过固定 fork 中不存在的 GLM-5.3 model-specific glue；把 DeepSeek DSA 的通用 pool/backend 误写成已经接通 GLM 的 E2E 路径，会掩盖真正待补的 config、model dispatch、layer composition 与权重加载 boundary。
...
| 新增 per-token sidecar | 让主 pool owner 持有 sidecar，并复用同一 loc；扩展 setter/getter/move | 再建一套 request mapping 或 allocator | sizing、finish/evict/reuse generation、offload/relocation（启用时） |
...
| 改 sparse candidate 表示 | indexer `topk_transform` 与 sparse backend contract | Radix value | raw/logical/physical index 语义、PAGED/RAGGED 两条路径 |
| 改 prefix 隔离策略 | `RadixKey` 的 namespace/`extra_key`/`cache_salt` 生成处 | physical setter | 相同 token 不同模型/adapter/layout 绝不误命中 |
...
- Lifecycle：prefix hit、unfinished cache、finish、evict、reuse、可能的 relocate/offload 都让主池与 sidecar 同步；free 后旧字节视为 stale，不靠清零保证正确性。

- Metadata：per-forward tensors 的有效范围完全由 lengths/indptr/mask 限定；CUDA-graph 静态 buffer 的尾部不得被当作 persistent state。

- Correctness：同一 prompt 的 cold run 与 prefix-hit run 输出等价；强制 eviction 后重算等价；反复 page reuse 下启用 generation/canary 检查无跨请求污染。
...
def build_dsa_views(
    ...
    write_locs: list[set[int]],
    shared_cached_locs: set[int],
) -> DSAViews:
    ...
```
...
- 对每个有效 loc，`latent_epoch[loc] == index_epoch[loc]`；缺项或主池/sidecar 世代不一致立即抛 `ValueError`。这里的 epoch 是测试用内容世代，不是源码的 request-row `req_generation`。

- `write_locs[i]` 是本轮第 i 个 request 的 private write set，必须属于该 request 的有效 row；不同 request 的 write sets 互斥，且任何 write loc 都不能出现在 `shared_cached_locs`。

- 不同 requests 可以同时读取同一个 cached-prefix loc，但重复 loc 必须显式列入 `shared_cached_locs`，并保持只读；未声明的跨 request alias、private write overlap 或对共享 loc 的写入立即抛 `ValueError`。
...
seq_lens = [70, 10]
page_size = 64
write_locs = [set(range(2624, 2630)), set(range(3328, 3338))]
shared_cached_locs = set(range(704, 768))
...
- 新增一个 request 复用 `704..767` 且这些 loc 已列入 `shared_cached_locs` 时必须接受；若移出共享集合、或把 loc 704 放入任一 request 的 `write_locs`，则必须拒绝。表构造过程中不得修改输入 rows。

### 渐进 hint

1. 先遍历有效 loc，用一个全局 `seen` 记录首次 owner；重复 loc 只有在显式 shared 且所有 request 都只读时才合法，同时检查主池/sidecar epoch，再处理 page chunks。
...
不用看文档，沿下面总线说一遍：`Radix prefix locs + allocator new locs -> ReqToTokenPool row -> ForwardBatch -> latent/indexer stores -> backend page tables/lengths -> indexer top-k transform -> attention reads latent -> finish optionally transfers or releases ownership -> eviction returns cached page IDs`。若任何箭头被说成“复制 KV”，回到六层状态表判断它究竟是 metadata 关系还是真实数据移动。

再核对三个容易静默出错的 invariant：`cached page` 不在 allocator free set；一个可见 DSA loc 的 latent 与 indexer sidecar 是同一 generation（这里是测试/正确性约束，不是源码中的独立 page-epoch 字段）；top-k result 属于 per-forward state，绝不作为 prefix persistent state。
...
在这个固定快照中，最值得守住的二次开发 boundary 是 `allocation 产出统一 flat loc -> pool 解释 payload layout -> backend 构造临时 kernel view -> Radix/allocator 统一管理 loc 生命周期`。MLA 只替换每个 loc 的主 payload，DSA 只增加与主 payload 同址同寿命的 sidecar；因此新增 cache/state 优化时，优先改 pool、capacity accounting 与 backend metadata contract，不让 Scheduler 或 Radix 理解模型张量细节，也不为 sidecar 复制一套 allocator。

## 下一篇预告

Day07 转入 Ironwood 的 ICI/OCS 互连与系统拓扑：从 chip、4-chip host、64-chip cube 到 Pod/Superpod，厘清 3D torus、OCS 与 DCN 的边界，以及带宽/拓扑数字的 scope。Linear Attention/KDA 与 ReplaySSM 不属于本系列下一篇；若继续研究它们，应另开软件运行时专题，避免把不同 Day 的主题串错。
